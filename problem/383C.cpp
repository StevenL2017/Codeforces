...
void solve() {
    ...
    sort(idx.begin(), idx.end(), [&] (int i, int j) { return tin[i] < tin[j]; });
    for (int i = 0; i < n; i++) {
        mp[idx[i]] = i;
    }
    ...
    while (m--) {
        ...
        if (op == 1) {
            ...
            x--;
            int i = mp[x];
            int l = upper_bound(c.begin(), c.end(), tin[x]) - c.begin() + 1;
            int r = lower_bound(c.begin(), c.end(), tout[x]) - c.begin();
            tree[dep[x]].update(1, l, r, val);
        } else {
            int x; cin >> x;
            x--;
            int i = mp[x];
            cout << b[x] + tree[dep[x]].query(1, i + 1, i + 1) - tree[dep[x] ^ 1].query(1, i + 1, i + 1) << endl;
        }
    }
}
